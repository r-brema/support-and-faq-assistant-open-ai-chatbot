...
from app.core.utils import md5, chunk
...
def _load_faq_json(path: str) -> List[Dict[str, Any]]:
    """
        ...
        for q in block["questions"]:
            ...
            for lang in ("fr", "en"):
                ...
                items.append({
                    ...
                    "faq_id": md5(f"{theme}|{lang}|{q[lang]}"),  
...
def _build_payload(items: List[Dict[str, Any]]):
    ...
    for it in items:
        ...
        rid = f"{lang}_{md5(f'{lang}|{theme}|{q}')}"

        # Append ID, document text, and metadata to respective lists
        ids.append(rid)
        documents.append(doc)
        metadatas.append({
            "faq_id": it["faq_id"],  # Original FAQ ID from _load_faq_json
            "lang": lang,            # Language
            "theme": theme,          # Theme/category
...
def _verify_ingestion(collection):
    """
    ...
    try:
        ...
        if faq_sample and "documents" in faq_sample:
            ...
            for i, doc in enumerate(faq_sample["documents"], 1):
                ...
                meta = faq_sample["metadatas"][i-1] if "metadatas" in faq_sample else {}

                # Log details: faq number, language, document text, and metadata
                logger.info(
                    "[faq no. %d] lang=%s | doc=%s | meta=%s",
                    i, meta.get("lang"), doc[:120], meta
                )
        else:
...
def run_ingestion():
    ...
    items = _load_faq_json(settings.JSON_FAQ_PATH)
    ...
    for batch_idx, idxs in enumerate(chunk(list(range(len(dedup_ids))), batch_size), 1):
        ...
        try:
            ...
            logger.exception("Chroma write failed for batch %d: %s", batch_idx, e)
   
    logger.info("All batches processed. Total items ingested: %d", ingested_count)
    _verify_ingestion(coll) # Peek 2 sample docs
