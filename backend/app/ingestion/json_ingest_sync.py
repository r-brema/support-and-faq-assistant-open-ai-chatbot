...
# -----------------------------
    ...
    batch = getattr(settings, "EMBEDDING_BATCH_SIZE", 16)
    coll = get_collection()
    ...
    # Process in batches
    total = len(dedup_ids)
    for idxs in tqdm(list(_chunk(list(range(total)), batch)), desc="Ingesting", unit="batch"):
        ...
        try:
            if hasattr(coll, "upsert"):
                coll.upsert(ids=b_ids, documents=b_docs, metadatas=b_meta, embeddings=embeddings)
            else:
    ...
    logger.info("✅ Ingestion complete into collection '%s' at '%s'",
                coll.name if hasattr(coll, "name") else "faqs",
                settings.CHROMA_DB_PATH)

    # Verify ingestion
    verify_ingestion(coll)
def run_ingestion():
    logger.info("Loading FAQ JSON from %s", settings.JSON_FAQ_PATH)
    ...
    for batch_idx, idxs in enumerate(_chunk(list(range(len(dedup_ids))), batch_size), 1):
        ...
        try:
            ...
            if hasattr(coll, "upsert"):
                ...
                coll.upsert(ids=b_ids, documents=b_docs, metadatas=b_meta, embeddings=embeddings)
                logger.info("updated embedding to collecctions"); 
            else:
...
#     persistent_path="C:/wamp64/www/portfolio-projects/AI/support-and-faq-assistant-open-ai-chatbot/backend/chroma_db/medical_link",
